      ...
      <ModalContent>
          ...
          <Flex justifyContent="space-between" flexDir="column">
            ...
            <StudentInfoItem
              ...
              value={
                student?.programme
                  ? student?.programme &&
                    Programme[student.programme].replaceAll("_", " ")
                  : "No program"
              }
            />
            <StudentInfoItem
            ...
            {user && (
              <Skeleton isLoaded={!loading}>
                <VStack align="strech" spacing="0" my="0.2rem">
                  ...
                  {cvUri ? (
                    <Link color="ArkadLightBlue" href={cvUri}>
                      Download CV
                    </Link>
                  ) : (
        ...
        <ModalFooter>
          <SimpleGrid columns={3} spacing="1rem" w="100%">
            ...
            {(application?.status === 2 || application?.status === 0) && (
              <Button variant="accept" onClick={accept}>
                Accept
              </Button>
            )}
            {(application?.status === 2 || application?.status === 1) && (
